local function __run()
    ...
    local function Hop()
            ...
            task.spawn(function()
                ...
                if ok and type(result)=='table' then
                    ...
                    for uuid,info in pairs(result) do
                        if type(info)=='table' and info.Count then allServers[uuid]=info; v+=1 end
                    end
                    if v>0 and not foundData then foundData=true; print('[ZBounty] ServerBrowser: '..v..' servers') end
                end
                pending -= 1
            end)
        end
        ...
        local waited=0
        while pending>0 and waited<6 do
        ...
        if foundData then
            for uuid,info in pairs(allServers) do
                if uuid~=_id then
                    local count=info.Count or 0; local bounty=info.Bounty or 0; local region=info.Region or ''
        ...
        local ok,_ = pcall(function() browser:InvokeServer('teleport',chosen.uuid) end)
    ...
    local function pressKey(key)
        VIM:SendKeyEvent(true,  key, false, game); task.wait(0.01)
    ...
    -- ============================================================
    --  NEAREST PLAYER — jugador mas cercano con nivel valido
    -- ============================================================
    ...
    -- ============================================================
    --  VOID SKILL — ataca al enemigo mas cercano desde el cielo
    -- ============================================================
    local function VoidSkill()
        ...
        if getgenv().CombatStyle == 'Sword' then
            equip('Sword'); equipByName('Sword')
        else
            equip('Melee'); equipByName('Melee')
        end
        task.wait(0.1)
        ...
        if targetHRP then
            -- Posicionarse ENCIMA del enemigo para que el Z lo alcance
            local tPos = targetHRP.Position
            hrp.CFrame = CFrame.new(tPos.X, tPos.Y + 80, tPos.Z)
            -- Mirar hacia el enemigo
            hrp.CFrame = CFrame.lookAt(hrp.Position, Vector3.new(tPos.X, hrp.Position.Y, tPos.Z))
        else
            -- Sin target: posicion infinita normal
        ...
        -- Volver a posicion original y desanclar
        hrp.CFrame = oldPos
        hrp.Anchored = false
    ...
    -- Loop respawn abuse original (orbita + Z normal)
    task.spawn(function()
        while task.wait() do
            ...
            if CONFIG.Weapon == 'funcion' then
                equip('Sword'); task.wait(0.15); down('Z'); State.lastZTime=os.clock(); task.wait(0.3)
                equip('Melee'); task.wait(0.15); down('Z'); State.lastZTime=os.clock(); task.wait(0.3)
            else
                equip(CONFIG.Weapon); task.wait(0.15); down('Z'); State.lastZTime=os.clock(); task.wait(0.5)
            end
        ...
        while task.wait(1) do
            ...
            if CONFIG.MaxServerTime and CONFIG.MaxServerTime>0 then
                if os.clock()-State.serverJoinTime >= CONFIG.MaxServerTime then
                    State.status='Tiempo limite → Hop'; saveData()
                    for i=1,5 do if Hop() then break end; task.wait(4) end
                    task.wait(3); State.serverJoinTime=os.clock(); State.lastHitTime=os.clock(); State.status='Activo'
                    continue
                end
            end
            if not hasValidTargets() then
                State.status='Sin targets → Hop'; task.wait(1)
                for i=1,5 do if Hop() then break end; task.wait(4) end
                task.wait(2); State.serverJoinTime=os.clock(); State.lastHitTime=os.clock(); State.status='Activo'
                continue
            end
    ...
    local function startAll()
        loadData()
        State.active=true; State.enabledCielo=true; State.autoHaki=true; State.respawnAbuse=true
        State.lastHitTime=os.clock(); State.currentBounty=getBounty(); State.status='Activo'
    ...
    local THEMES = {
        ...
        Orange ={accent=Color3.fromRGB(240,130,40), bg=Color3.fromRGB(14,10,6),  panel=Color3.fromRGB(18,13,8), card=Color3.fromRGB(20,15,9)},
        ...
        Blue   ={accent=Color3.fromRGB(60,120,255), bg=Color3.fromRGB(7,8,18),   panel=Color3.fromRGB(9,10,22), card=Color3.fromRGB(11,12,26)},
        ...
        Pink   ={accent=Color3.fromRGB(240,60,180), bg=Color3.fromRGB(14,7,13),  panel=Color3.fromRGB(18,9,16), card=Color3.fromRGB(20,10,18)},
    }
    local THEME=THEMES[CONFIG.Theme] or THEMES.Default
    local T_ACCENT=THEME.accent; local T_BG=THEME.bg; local T_PANEL=THEME.panel; local T_CARD=THEME.card
    local C={
        bg=T_BG,panel=T_PANEL,card=T_CARD,
        border=T_ACCENT:Lerp(Color3.fromRGB(5,5,10),0.75),
        cyan=Color3.fromRGB(0,190,240),green=Color3.fromRGB(45,210,110),
        red=Color3.fromRGB(215,60,60),gold=Color3.fromRGB(240,185,55),
        text=Color3.fromRGB(220,225,245),muted=Color3.fromRGB(90,100,135),
    }

    local function addStroke(p,color,thick,trans)
        local s=Instance.new('UIStroke',p); s.Color=color or C.border; s.Thickness=thick or 1; s.Transparency=trans or 0; return s
    end
    local function mkFrame(parent,size,pos,bg,trans)
        local f=Instance.new('Frame',parent); f.Size=size; f.Position=pos or UDim2.new(0,0,0,0)
        f.BackgroundColor3=bg or C.card; f.BackgroundTransparency=trans or 0; f.BorderSizePixel=0
    ...
    local function mkLabel(parent,size,pos,text,font,textSize,color,xAlign)
        local l=Instance.new('TextLabel',parent); l.Size=size; l.Position=pos or UDim2.new(0,0,0,0)
        l.BackgroundTransparency=1; l.Text=text or ''; l.Font=font or Enum.Font.Gotham
        l.TextSize=textSize or 11; l.TextColor3=color or C.text
        l.TextXAlignment=xAlign or Enum.TextXAlignment.Center; l.TextTruncate=Enum.TextTruncate.AtEnd; return l
    end

    for _,parent in ipairs({player.PlayerGui,game:GetService('CoreGui')}) do
        pcall(function() local old=parent:FindFirstChild('ZBountyUI'); if old then old:Destroy() end end)
    end

    local ScreenGui=Instance.new('ScreenGui')
    ScreenGui.Name='ZBountyUI'; ScreenGui.ResetOnSpawn=false
    ...
    ToggleBtn.Size=UDim2.new(0,38,0,38); ToggleBtn.Position=UDim2.new(0,8,0,8)
    ToggleBtn.BackgroundColor3=T_BG; ToggleBtn.Text=''; ToggleBtn.BorderSizePixel=0; ToggleBtn.AutoButtonColor=false
    Instance.new('UICorner',ToggleBtn).CornerRadius=UDim.new(0,10)
    addStroke(ToggleBtn,T_ACCENT,1,0.3)
    local tli=Instance.new('ImageLabel',ToggleBtn); tli.Size=UDim2.new(1,-6,1,-6); tli.Position=UDim2.new(0,3,0,3)
    tli.BackgroundTransparency=1; tli.ScaleType=Enum.ScaleType.Fit; tli.Image='rbxassetid://125174382377001'; tli.ImageColor3=T_ACCENT

    local Main=Instance.new('Frame',ScreenGui)
    Main.Name='ZBountyMain'; Main.Size=UDim2.new(0,580,0,340); Main.Position=UDim2.new(0,52,0,8)
    Main.BackgroundColor3=T_BG; Main.BackgroundTransparency=0; Main.BorderSizePixel=0
    Main.Active=true; Main.Draggable=true; Main.Visible=false
    Instance.new('UICorner',Main).CornerRadius=UDim.new(0,10)
    local mainStroke=addStroke(Main,T_ACCENT,1,0.6)
    task.spawn(function()
        local t=0
        while true do task.wait(0.06); t+=0.06
            if mainStroke and mainStroke.Parent then mainStroke.Transparency=0.45+0.3*math.abs(math.sin(t*0.8)) end
        end
    end)

    local left=Instance.new('Frame',Main)
    left.Size=UDim2.new(0,168,1,0); left.BackgroundColor3=C.panel; left.BackgroundTransparency=0.3; left.BorderSizePixel=0
    Instance.new('UICorner',left).CornerRadius=UDim.new(0,10); addStroke(left,T_ACCENT,1,0.55)
    local lFix=Instance.new('Frame',left); lFix.Size=UDim2.new(0,12,1,0); lFix.Position=UDim2.new(1,-12,0,0); lFix.BackgroundColor3=T_PANEL; lFix.BackgroundTransparency=0.3; lFix.BorderSizePixel=0

    local function lDiv(y)
        local d=Instance.new('Frame',left); d.Size=UDim2.new(0.75,0,0,1); d.Position=UDim2.new(0.125,0,0,y)
        d.BackgroundColor3=T_ACCENT; d.BackgroundTransparency=0.75; d.BorderSizePixel=0
    end

    local logoFrame=Instance.new('Frame',left); logoFrame.Size=UDim2.new(0,80,0,80); logoFrame.Position=UDim2.new(0.5,-40,0,10); logoFrame.BackgroundTransparency=1; logoFrame.BorderSizePixel=0
    local logoImg=Instance.new('ImageLabel',logoFrame); logoImg.Size=UDim2.new(1,0,1,0); logoImg.BackgroundTransparency=1; logoImg.Image='rbxassetid://125174382377001'; logoImg.ScaleType=Enum.ScaleType.Fit; logoImg.ImageColor3=T_ACCENT

    local avOuter=Instance.new('Frame',left); avOuter.Size=UDim2.new(0,70,0,70); avOuter.Position=UDim2.new(0.5,-35,0,98); avOuter.BackgroundColor3=T_BG; avOuter.BorderSizePixel=0
    Instance.new('UICorner',avOuter).CornerRadius=UDim.new(1,0); addStroke(avOuter,T_ACCENT,2,0.2)
    local avImg=Instance.new('ImageLabel',avOuter); avImg.Size=UDim2.new(1,-4,1,-4); avImg.Position=UDim2.new(0,2,0,2); avImg.BackgroundTransparency=1; avImg.ScaleType=Enum.ScaleType.Crop
    Instance.new('UICorner',avImg).CornerRadius=UDim.new(1,0)
    task.spawn(function()
        local ok3,url=pcall(function() return Players:GetUserThumbnailAsync(player.UserId,Enum.ThumbnailType.HeadShot,Enum.ThumbnailSize.Size150x150) end)
    ...
    local factionBg=mkFrame(left,UDim2.new(0.88,0,0,24),UDim2.new(0.06,0,0,285),CONFIG.Team=='Pirates' and Color3.fromRGB(100,22,22) or Color3.fromRGB(20,55,110),0)
    addStroke(factionBg,CONFIG.Team=='Pirates' and Color3.fromRGB(210,80,80) or Color3.fromRGB(80,150,230),1,0.35)
    mkLabel(factionBg,UDim2.new(1,0,1,0),UDim2.new(0,0,0,0),CONFIG.Team=='Pirates' and '🏴 PIRATAS' or '⚓ MARINES',Enum.Font.GothamBlack,12,Color3.new(1,1,1))
    lDiv(313)
    local regionLbl=mkLabel(left,UDim2.new(1,-8,0,14),UDim2.new(0,4,0,317),'? / 12 jugadores',Enum.Font.Gotham,9,C.muted)

    local right=Instance.new('Frame',Main)
    right.Size=UDim2.new(1,-170,1,0); right.Position=UDim2.new(0,170,0,0)
    right.BackgroundColor3=T_BG; right.BackgroundTransparency=0.45; right.BorderSizePixel=0
    ...
    local hFix=Instance.new('Frame',headerBar); hFix.Size=UDim2.new(1,0,0,10); hFix.Position=UDim2.new(0,0,1,-10); hFix.BackgroundColor3=T_CARD; hFix.BorderSizePixel=0
    local statusDot=Instance.new('Frame',headerBar); statusDot.Size=UDim2.new(0,7,0,7); statusDot.Position=UDim2.new(0,8,0.5,-3.5); statusDot.BackgroundColor3=C.green; statusDot.BorderSizePixel=0
    ...
    TimerLbl=mkLabel(headerBar,UDim2.new(0,65,1,0),UDim2.new(0,141,0,0),'⏱ 0s',Enum.Font.GothamBold,12,C.muted)
    ...
    local hActivoLbl=mkLabel(hActivoBadge,UDim2.new(1,0,1,0),UDim2.new(0,0,0,0),'● CARGANDO',Enum.Font.GothamBold,9,C.green)

    local discordBtn=Instance.new('TextButton',headerBar)
    discordBtn.Size=UDim2.new(0,74,0,22); discordBtn.Position=UDim2.new(0,288,0.5,-11)
    discordBtn.BackgroundColor3=Color3.fromRGB(40,46,120); discordBtn.BorderSizePixel=0
    discordBtn.Text='💬 Discord'; discordBtn.Font=Enum.Font.GothamBold; discordBtn.TextSize=9; discordBtn.TextColor3=Color3.new(1,1,1); discordBtn.AutoButtonColor=false
    Instance.new('UICorner',discordBtn).CornerRadius=UDim.new(0,5); addStroke(discordBtn,Color3.fromRGB(110,120,235),1,0.3)
    discordBtn.MouseButton1Click:Connect(function()
        pcall(function() setclipboard('https://discord.gg/NYJC6WfVsJ') end)
        local old=discordBtn.Text; discordBtn.Text='✓ Copiado!'; task.wait(2); discordBtn.Text=old
    end)

    local minBtn=Instance.new('TextButton',headerBar)
    minBtn.Size=UDim2.new(0,24,0,20); minBtn.Position=UDim2.new(1,-28,0.5,-10)
    minBtn.BackgroundColor3=Color3.fromRGB(80,20,20); minBtn.Text='−'; minBtn.TextColor3=Color3.new(1,1,1)
    minBtn.Font=Enum.Font.GothamBlack; minBtn.TextSize=14; minBtn.BorderSizePixel=0; minBtn.AutoButtonColor=false
    Instance.new('UICorner',minBtn).CornerRadius=UDim.new(0,5); addStroke(minBtn,Color3.fromRGB(200,70,70),1,0.4)
    ...
    local function statCell(parent,label,idx,total,valColor)
        local w=1/total
        local cell=Instance.new('Frame',parent); cell.Size=UDim2.new(w,0,1,0); cell.Position=UDim2.new(w*idx,0,0,0); cell.BackgroundTransparency=1; cell.BorderSizePixel=0
        if idx>0 then local sep=Instance.new('Frame',cell); sep.Size=UDim2.new(0,1,0.45,0); sep.Position=UDim2.new(0,0,0.275,0); sep.BackgroundColor3=T_ACCENT; sep.BackgroundTransparency=0.7; sep.BorderSizePixel=0 end
        mkLabel(cell,UDim2.new(1,0,0,13),UDim2.new(0,0,0,3),label,Enum.Font.Gotham,8,C.muted)
        local val=mkLabel(cell,UDim2.new(1,0,0,22),UDim2.new(0,0,0,16),'—',Enum.Font.GothamBlack,15,valColor or C.text)
        return val
    end
    ...
    TimerBar=Instance.new('Frame',barBg); TimerBar.Size=UDim2.new(0,0,1,0); TimerBar.BackgroundColor3=T_ACCENT; TimerBar.BorderSizePixel=0
    Instance.new('UICorner',TimerBar).CornerRadius=UDim.new(0,5)
    local tgrad=Instance.new('UIGradient',TimerBar)
    tgrad.Color=ColorSequence.new({ColorSequenceKeypoint.new(0,T_ACCENT),ColorSequenceKeypoint.new(1,T_ACCENT:Lerp(Color3.new(1,1,1),0.3))})
    local barPctLbl=mkLabel(barBg,UDim2.new(1,0,1,0),UDim2.new(0,0,0,0),'0s',Enum.Font.GothamBold,9,C.text); barPctLbl.ZIndex=2

    local tlHdr=mkFrame(right,UDim2.new(1,-6,0,16),UDim2.new(0,3,0,141),T_CARD,0)
    addStroke(tlHdr,T_ACCENT,1,0.6)
    mkLabel(tlHdr,UDim2.new(0.42,0,1,0),UDim2.new(0,0,0,0),'TARGET LIST',Enum.Font.GothamBold,8,C.gold,Enum.TextXAlignment.Left)
    mkLabel(tlHdr,UDim2.new(0.3,0,1,0),UDim2.new(0.42,0,0,0),'HP BAR',Enum.Font.GothamBold,8,C.gold,Enum.TextXAlignment.Center)
    mkLabel(tlHdr,UDim2.new(0.28,0,1,0),UDim2.new(0.72,0,0,0),'BOUNTY GAIN',Enum.Font.GothamBold,8,C.gold,Enum.TextXAlignment.Right)

    local targetScroll=Instance.new('ScrollingFrame',right)
    targetScroll.Size=UDim2.new(1,-6,1,-235); targetScroll.Position=UDim2.new(0,3,0,159)
    targetScroll.BackgroundTransparency=1; targetScroll.BorderSizePixel=0; targetScroll.ScrollBarThickness=2
    targetScroll.ScrollBarImageColor3=T_ACCENT; targetScroll.CanvasSize=UDim2.new(0,0,0,0); targetScroll.AutomaticCanvasSize=Enum.AutomaticSize.Y
    local tlLayout=Instance.new('UIListLayout',targetScroll); tlLayout.SortOrder=Enum.SortOrder.LayoutOrder; tlLayout.Padding=UDim.new(0,2)
    ...
    local function rebuildTargetList()
        for _,ch in pairs(targetScroll:GetChildren()) do if ch:IsA('Frame') then ch:Destroy() end end
        targetRows={}
        local list={}
        ...
        table.sort(list,function(a,b)
            ...
            local la,lb=0,0
            pcall(function() la=a.Data.Level.Value end); pcall(function() lb=b.Data.Level.Value end)
            return la>lb
        end)
        for order,pl in ipairs(list) do
            ...
            row.BackgroundColor3=isActive and T_BG:Lerp(T_ACCENT,0.08) or T_CARD; row.BackgroundTransparency=0; row.BorderSizePixel=0
            Instance.new('UICorner',row).CornerRadius=UDim.new(0,6); addStroke(row,T_ACCENT,1,isActive and 0.2 or 0.75)
            ...
            local lvVal=0; pcall(function() lvVal=pl.Data.Level.Value end)
            ...
            local hpBg=Instance.new('Frame',row); hpBg.Size=UDim2.new(0.28,0,0,10); hpBg.Position=UDim2.new(0.42,0,0.5,-5); hpBg.BackgroundColor3=T_BG; hpBg.BorderSizePixel=0
            Instance.new('UICorner',hpBg).CornerRadius=UDim.new(0,3); addStroke(hpBg,T_ACCENT,1,0.6)
            local hpFill=Instance.new('Frame',hpBg); hpFill.Name='HPFill'; hpFill.Size=UDim2.new(1,0,1,0); hpFill.BackgroundColor3=C.green; hpFill.BorderSizePixel=0
            Instance.new('UICorner',hpFill).CornerRadius=UDim.new(0,3)
            local hpPct=mkLabel(hpBg,UDim2.new(1,0,1,0),UDim2.new(0,0,0,0),'100%',Enum.Font.GothamBold,7,Color3.new(1,1,1)); hpPct.Name='HPPct'; hpPct.ZIndex=2
    ...
    task.spawn(function()
        local rebuildT=0
        while true do
            task.wait(0.25); rebuildT+=0.25
            if rebuildT>=3 then rebuildT=0; pcall(rebuildTargetList) end
            for _,data in pairs(targetRows) do
                pcall(function()
                    local hum=data.hum; if not hum or not hum.Parent then return end
            ...
            pcall(function()
                ...
                local locs=workspace:FindFirstChild('_WorldOrigin'); if locs then locs=locs:FindFirstChild('Locations') end; if not locs then return end
                local best,bestD=nil,math.huge
                for _,loc in pairs(locs:GetChildren()) do local d=(loc.Position-hrp.Position).Magnitude; if d<bestD then best=loc; bestD=d end end
            ...
            pcall(function()
                ...
                pingVal.Text=p..'ms'; pingVal.TextColor3=p<100 and C.green or (p<250 and C.gold or C.red)
            end)
            pcall(function() local lv=0; pcall(function() lv=player.Data.Level.Value end); lvlLbl.Text='Lv. '..lv end)
    ...
    miniBar.Name='ZBountyMini'; miniBar.Size=UDim2.new(0,400,0,30); miniBar.Position=UDim2.new(0.5,-200,0,10)
    miniBar.BackgroundColor3=T_BG; miniBar.BackgroundTransparency=0.1; miniBar.BorderSizePixel=0; miniBar.Visible=false; miniBar.Active=true; miniBar.Draggable=true
    Instance.new('UICorner',miniBar).CornerRadius=UDim.new(0,15); addStroke(miniBar,T_ACCENT,1,0.3)
    local mli=Instance.new('ImageLabel',miniBar); mli.Size=UDim2.new(0,24,0,24); mli.Position=UDim2.new(0,4,0.5,-12); mli.BackgroundTransparency=1; mli.ScaleType=Enum.ScaleType.Fit; mli.Image='rbxassetid://125174382377001'; mli.ImageColor3=T_ACCENT
    ...
    local function mSep(x) local s=Instance.new('Frame',miniBar); s.Size=UDim2.new(0,1,0.4,0); s.Position=UDim2.new(0,x,0.3,0); s.BackgroundColor3=T_ACCENT; s.BackgroundTransparency=0.7; s.BorderSizePixel=0 end
    local function mStat(x,w,color) return mkLabel(miniBar,UDim2.new(0,w,1,0),UDim2.new(0,x,0,0),'—',Enum.Font.GothamBold,11,color) end

    mSep(155); local mKills=mStat(159,65,C.text)
    mSep(226); local mBounty=mStat(230,80,C.green)
    mSep(312); local mPing=mStat(316,60,T_ACCENT)
    mSep(378); local mFPS=mStat(382,55,C.gold)

    local _fc,_fv,_fl=0,0,tick()
    RunService.RenderStepped:Connect(function() _fc+=1; local n=tick(); if n-_fl>=1 then _fv=_fc; _fc=0; _fl=n end end)
    task.spawn(function()
        while true do task.wait(0.5)
            ...
            pcall(function()
                ...
                local p=0; pcall(function() p=math.floor(game:GetService('Stats').Network.ServerStatsItem['Data Ping']:GetValue()) end)
    ...
    -- ============================================================
    --  INF AUTO PANEL (abajo del scroll)
    -- ============================================================
    getgenv().InfAutoActive = false
    getgenv().CombatStyle   = 'Melee'

    local infPanel=mkFrame(right,UDim2.new(1,-6,0,70),UDim2.new(0,3,1,-74),T_CARD,0)
    addStroke(infPanel,T_ACCENT,1,0.5)
    mkLabel(infPanel,UDim2.new(1,0,0,14),UDim2.new(0,0,0,4),'INF AUTO  |  ESTILO  |  TARGET: JUGADOR MAS CERCANO',Enum.Font.GothamBold,8,C.muted)

    local infBtn=Instance.new('TextButton',infPanel)
    infBtn.Size=UDim2.new(0.45,0,0,28); infBtn.Position=UDim2.new(0,4,0,18)
    infBtn.BackgroundColor3=T_BG:Lerp(T_ACCENT,0.1); infBtn.Text='⚡ INF: OFF'
    infBtn.TextColor3=C.muted; infBtn.Font=Enum.Font.GothamBlack; infBtn.TextSize=11; infBtn.BorderSizePixel=0; infBtn.AutoButtonColor=false
    Instance.new('UICorner',infBtn).CornerRadius=UDim.new(0,6); addStroke(infBtn,T_ACCENT,1,0.5)

    local meleeBtn=Instance.new('TextButton',infPanel)
    meleeBtn.Size=UDim2.new(0.24,-3,0,28); meleeBtn.Position=UDim2.new(0.47,2,0,18)
    meleeBtn.BackgroundColor3=T_BG:Lerp(C.green,0.2); meleeBtn.Text='👊 Melee'
    meleeBtn.TextColor3=C.green; meleeBtn.Font=Enum.Font.GothamBlack; meleeBtn.TextSize=10; meleeBtn.BorderSizePixel=0; meleeBtn.AutoButtonColor=false
    Instance.new('UICorner',meleeBtn).CornerRadius=UDim.new(0,6); addStroke(meleeBtn,C.green,1,0.3)

    local swordBtn=Instance.new('TextButton',infPanel)
    swordBtn.Size=UDim2.new(0.24,-3,0,28); swordBtn.Position=UDim2.new(0.74,2,0,18)
    swordBtn.BackgroundColor3=T_BG:Lerp(C.gold,0.1); swordBtn.Text='⚔️ Sword'
    swordBtn.TextColor3=C.muted; swordBtn.Font=Enum.Font.GothamBlack; swordBtn.TextSize=10; swordBtn.BorderSizePixel=0; swordBtn.AutoButtonColor=false
    Instance.new('UICorner',swordBtn).CornerRadius=UDim.new(0,6); addStroke(swordBtn,C.gold,1,0.7)

    local infStatusLbl=mkLabel(infPanel,UDim2.new(1,-8,0,12),UDim2.new(0,4,0,52),'● Apuntando al jugador mas cercano con nivel '..CONFIG.MinLevel..'+',Enum.Font.Gotham,9,C.muted)

    local function updateStyleBtns()
        if getgenv().CombatStyle=='Melee' then
            meleeBtn.TextColor3=C.green; meleeBtn.BackgroundColor3=T_BG:Lerp(C.green,0.2); addStroke(meleeBtn,C.green,1,0.3)
            swordBtn.TextColor3=C.muted; swordBtn.BackgroundColor3=T_BG:Lerp(C.gold,0.05)
            infStatusLbl.Text='● Melee — apunta al jugador mas cercano Lv.'..CONFIG.MinLevel..'+'
        else
            swordBtn.TextColor3=C.gold; swordBtn.BackgroundColor3=T_BG:Lerp(C.gold,0.2); addStroke(swordBtn,C.gold,1,0.3)
            meleeBtn.TextColor3=C.muted; meleeBtn.BackgroundColor3=T_BG:Lerp(C.green,0.05)
            infStatusLbl.Text='● Sword — apunta al jugador mas cercano Lv.'..CONFIG.MinLevel..'+'
        end
    end
    ...
    infBtn.MouseButton1Click:Connect(function()
        ...
        if getgenv().InfAutoActive then
            infBtn.Text='⚡ INF: ON'; infBtn.TextColor3=T_ACCENT; infBtn.BackgroundColor3=T_BG:Lerp(T_ACCENT,0.22)
        else
            infBtn.Text='⚡ INF: OFF'; infBtn.TextColor3=C.muted; infBtn.BackgroundColor3=T_BG:Lerp(T_ACCENT,0.1)
        end
    end)
    updateStyleBtns()
    ...
    local function openMain()
        miniBar.Visible=false; minimized=false; minBtn.Text='−'; Main.Visible=true
        Main.BackgroundTransparency=1; Main.Size=UDim2.new(0,580,0,0); Main.Position=UDim2.new(0,52,0,-10); Main.BackgroundColor3=T_BG
        TweenService:Create(Main,TweenInfo.new(0.22,Enum.EasingStyle.Quint,Enum.EasingDirection.Out),{Size=UDim2.new(0,580,0,340),BackgroundTransparency=0,Position=UDim2.new(0,52,0,8)}):Play()
    end
    local function closeMain(showMini)
        TweenService:Create(Main,TweenInfo.new(0.18,Enum.EasingStyle.Quint,Enum.EasingDirection.In),{Size=UDim2.new(0,580,0,0),BackgroundTransparency=1,Position=UDim2.new(0,52,0,-10)}):Play()
        task.wait(0.18); Main.Visible=false; Main.Size=UDim2.new(0,580,0,340); Main.BackgroundTransparency=0; Main.Position=UDim2.new(0,52,0,8)
        if showMini then miniBar.Visible=true end
    end

    miniBar.InputBegan:Connect(function(inp)
        if inp.UserInputType==Enum.UserInputType.Touch or inp.UserInputType==Enum.UserInputType.MouseButton1 then openMain() end
    end)
        ...
        while true do task.wait(0.5)
            pcall(function()
                ...
                if State.active then
                    ...
                    tgrad.Color=ColorSequence.new({ColorSequenceKeypoint.new(0,pct>0.4 and T_ACCENT or C.red),ColorSequenceKeypoint.new(1,pct>0.4 and T_ACCENT:Lerp(Color3.new(1,1,1),0.25) or C.gold)})
                    ...
                    hActivoLbl.Text='● '..string.upper(State.status)
    ...
    Main.Visible=false; guiReady=true

    task.spawn(function()
        print('[ZBounty] Iniciando — Team='..CONFIG.Team..' Weapon='..CONFIG.Weapon)
        local confirmed=false; local elapsed=0
        if player.Team and player.Team.Name==CONFIG.Team then confirmed=true
        elseif player.Team then print('[ZBounty] Team actual: '..player.Team.Name)
        else print('[ZBounty] Sin team — intentando...') end
        ...
        hActivoLbl.Text=ok3 and '● ACTIVO' or '● SIN TEAM'
        ...
        startAll(); task.wait(2)

        if not hasValidTargets() then
            print('[ZBounty] Sin targets Lv.'..CONFIG.MinLevel..'+ → Hopeando')
            State.status='Sin targets'; task.wait(2); Hop(); task.wait(5)
            State.lastHitTime=os.clock(); State.status='Activo'
        end
    end)
    ...
    task.spawn(function()
        ...
        while true do
            ...
            if vis and not lastVis then
                print('[ZBounty] Nuevo server → seleccionando '..CONFIG.Team); task.wait(0.4)
                for i=1,5 do selectFaction(CONFIG.Team); task.wait(1.2); if player.Team and player.Team.Name==CONFIG.Team then break end end
            end
            lastVis=vis
        end
    end)
