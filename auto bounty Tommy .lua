local function __run()
    local _user = getgenv and getgenv().TommyHubConfig or {}
    local CONFIG = {
    ...
    -- INF slot (siempre melee, slot 3 por defecto)
    local InfSlot = Enum.KeyCode.Three
    ...
    local SAVE_FILE = 'tommyhub_bounty.json'
    ...
    local function Hop()
        ...
        local allServers={}, foundData=false, pendingCount=0
        for page=1,100 do
            ...
            task.spawn(function()
                ...
                if ok and type(result)=='table' then
                    ...
                    if v>0 and not foundData then foundData=true end
                end
                pendingCount-=1
            end)
        end
    ...
    local function PressKey(key)
        pcall(function()
            VIM:SendKeyEvent(true,  key, false, game); task.wait(0.08)
            VIM:SendKeyEvent(false, key, false, game)
        end)
    end

    -- equipar melee (nunca sword)
        ...
        pcall(function()
            ...
            for _,tool in pairs(player.Backpack:GetChildren()) do
                if tool:IsA('Tool') then
                    ...
                    local tip = (tool.ToolTip or ''):lower()
                    if (tip=='melee' or n:find('fist') or n:find('melee') or n:find('combat') or n:find('fighting'))
                        and not n:find('sword') and not tip:find('sword') then
    ...
    task.spawn(function()
        ...
        while true do
            ...
            if _prevBounty>0 and cur<_prevBounty then
                State.status='Bounty perdido \u{2192} Hop'
        ...
        while task.wait(1) do
            ...
            if CONFIG.MaxServerTime and CONFIG.MaxServerTime>0 then
                if os.clock()-State.serverJoinTime>=CONFIG.MaxServerTime then
                    State.status='Tiempo limite \u{2192} Hop'; saveData()
            ...
            if not hasValidTargets() then
                State.status='Sin targets \u{2192} Hop'; task.wait(1)
                for i=1,5 do if Hop() then break end; task.wait(4) end
    ...
    for _,parent in ipairs({player.PlayerGui, game:GetService('CoreGui')}) do
        pcall(function() local old=parent:FindFirstChild('TommyHubUI'); if old then old:Destroy() end end)
    end

    local ScreenGui=Instance.new('ScreenGui')
    ScreenGui.Name='TommyHubUI'; ScreenGui.ResetOnSpawn=false
    ...
    toggleLogoImg.Size=UDim2.new(1,-6,1,-6); toggleLogoImg.Position=UDim2.new(0,3,0,3)
    toggleLogoImg.BackgroundTransparency=1; toggleLogoImg.ScaleType=Enum.ScaleType.Fit
    toggleLogoImg.Image='rbxassetid://125174382377001'; toggleLogoImg.ImageColor3=T_ACCENT

    -- Main frame
    local Main=Instance.new('Frame',ScreenGui)
    Main.Name='TommyHubMain'; Main.Size=UDim2.new(0,580,0,340)
    Main.Position=UDim2.new(0,52,0,8); Main.BackgroundColor3=T_BG
    ...
    lFix.Size=UDim2.new(0,12,1,0); lFix.Position=UDim2.new(1,-12,0,0)
    lFix.BackgroundColor3=T_PANEL; lFix.BackgroundTransparency=0.3; lFix.BorderSizePixel=0
    ...
    -- Logo / imagen del script
    local logoFrame=Instance.new('Frame',left)
    logoFrame.Size=UDim2.new(0,80,0,80); logoFrame.Position=UDim2.new(0.5,-40,0,10)
    logoFrame.BackgroundTransparency=1; logoFrame.BorderSizePixel=0
    local logoImg=Instance.new('ImageLabel',logoFrame)
    logoImg.Size=UDim2.new(1,0,1,0); logoImg.BackgroundTransparency=1
    logoImg.Image='rbxassetid://125174382377001'; logoImg.ScaleType=Enum.ScaleType.Fit; logoImg.ImageColor3=T_ACCENT

    -- Avatar del player
    local avOuter=Instance.new('Frame',left)
    avOuter.Size=UDim2.new(0,70,0,70); avOuter.Position=UDim2.new(0.5,-35,0,98)
    avOuter.BackgroundColor3=T_BG; avOuter.BorderSizePixel=0
    ...
    -- Badge INF (unica opcion visible)
    local infBadge=mkFrame(left,UDim2.new(0.88,0,0,24),UDim2.new(0.06,0,0,285),T_BG:Lerp(T_ACCENT,0.12),0)
    addStroke(infBadge,T_ACCENT,1,0.3)
    mkLabel(infBadge,UDim2.new(1,0,1,0),UDim2.new(0,0,0,0),'\u{26a1} INF MELEE',Enum.Font.GothamBlack,12,T_ACCENT)
    lDiv(313)

    -- Faccion
    local factionBg=mkFrame(left,UDim2.new(0.88,0,0,24),UDim2.new(0.06,0,1,-60),
    ...
    -- Jugadores en server
    local regionLbl=mkLabel(left,UDim2.new(1,-8,0,14),UDim2.new(0,4,1,-28),'? / 12 jugadores',Enum.Font.Gotham,9,C.muted)
    ...
    -- NOMBRE: Tommy Hub Bounty
    mkLabel(headerBar,UDim2.new(0,150,1,0),UDim2.new(0,18,0,0),'Tommy Hub Bounty',Enum.Font.GothamBlack,13,C.text,Enum.TextXAlignment.Left)

    TimerLbl=mkLabel(headerBar,UDim2.new(0,65,1,0),UDim2.new(0,172,0,0),'\u{23f1} 0s',Enum.Font.GothamBold,12,C.muted)
    local hActivoBadge=mkFrame(headerBar,UDim2.new(0,72,0,20),UDim2.new(0,240,0.5,-10),T_BG:Lerp(T_ACCENT,0.15),0)
    ...
    minBtn.BackgroundColor3=Color3.fromRGB(80,20,20); minBtn.Text='\u{2212}'
    minBtn.TextColor3=Color3.new(1,1,1); minBtn.Font=Enum.Font.GothamBlack; minBtn.TextSize=14
    ...
    local function statCell(parent,label,idx,total,valColor)
        ...
        if idx>0 then
            local sep=Instance.new('Frame',cell); sep.Size=UDim2.new(0,1,0.45,0); sep.Position=UDim2.new(0,0,0.275,0)
    ...
    targetScroll.Size=UDim2.new(1,-6,1,-161); targetScroll.Position=UDim2.new(0,3,0,159)
    ...
    miniBar.Name='TommyHubMini'; miniBar.Size=UDim2.new(0,400,0,30); miniBar.Position=UDim2.new(0.5,-200,0,10)
    ...
    miniLogoImg.Size=UDim2.new(0,24,0,24); miniLogoImg.Position=UDim2.new(0,4,0.5,-12)
    miniLogoImg.BackgroundTransparency=1; miniLogoImg.ScaleType=Enum.ScaleType.Fit
    miniLogoImg.Image='rbxassetid://125174382377001'; miniLogoImg.ImageColor3=T_ACCENT
    ...
    local function mStat(x,w,color) return mkLabel(miniBar,UDim2.new(0,w,1,0),UDim2.new(0,x,0,0),'\u{2014}',Enum.Font.GothamBold,11,color) end
    ...
    local function openMain()
        miniBar.Visible=false; minimized=false; minBtn.Text='\u{2212}'; Main.Visible=true
    ...
    Main.Visible=true; guiReady=true
    ...
    local function startAll()
        ...
        print('[Tommy Hub] ACTIVO \u{2014} '..CONFIG.Team..' / INF MELEE')
    end

    task.spawn(function()
        print('[Tommy Hub] Iniciando \u{2014} Team='..CONFIG.Team)
        local confirmed=false; local elapsed=0
        if player.Team and player.Team.Name==CONFIG.Team then confirmed=true
        elseif not player.Team then print('[Tommy Hub] Sin team \u{2014} intentando...') end
    ...
    task.spawn(function()
        ...
        while true do
            ...
            if vis and not lastVis then
                print('[Tommy Hub] Nuevo server \u{2192} seleccionando '..CONFIG.Team); task.wait(0.4)
    ...
    print('[Tommy Hub] Loaded \u{2014} INF MELEE activo')
end
